...
public class ExampleMod implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        // Registra il controllo al momento del click su un'entità
        UseEntityCallback.EVENT.register((player, world, hand, entity, hitResult) -> {
            // Controlla se l'oggetto toccato è un Item Frame
            if (entity instanceof ItemFrameEntity) {
                long currentTime = System.currentTimeMillis();
                int currentEntityId = entity.getId();

                // Se è il primo click su questo Item Frame negli ultimi 3 secondi
                if (currentEntityId != lastEntityId || (currentTime - lastFrameClickTime) > 3000) {
                    ...
                    // Blochiamo il pacchetto inviato al server
